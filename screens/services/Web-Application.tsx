                ...
                items={[
                    {
                        ...
                        body: `Subscription-based SaaS applications with enterprise-grade multi-tenancy, user management, and data isolation. We architect secure, scalable systems supporting unlimited customer growth with zero downtime. Features include: sophisticated billing & metering engines (per-user, per-feature, usage-based), role-based access control (RBAC) with granular permissions, team collaboration with real-time synchronization, comprehensive audit trails for compliance, advanced analytics dashboards tracking business metrics, and automated reporting. Our platforms handle millions of transactions daily with sub-100ms response times, 99.99% uptime SLAs, and automatic scaling infrastructure. Integration with payment processors (Stripe, Square), identity providers (Auth0, Okta), and cloud services (AWS, Azure, GCP) is built in. We've delivered SaaS platforms for HR tech, project management, CRM, analytics, and compliance domains—each handling 5M+ monthly active users with proven retention and expansion metrics.`,
                        tags: ["Multi-tenant", "RBAC", "Billing Engine", "Usage Metering", "Compliance", "99.99% Uptime"],
                    },
                    {
                        id: "PWA",
                        target: "PWA",
                        title: "Progressive Web Apps (PWA)",
                        body: `Progressive Web Apps that blur the line between web and native, delivering app-like experiences directly in the browser without app store friction. We build PWAs using modern web standards: service workers for offline-first functionality and background sync, Web App Manifest for home screen installation with custom branding, responsive design frameworks for all device sizes, and aggressive caching strategies ensuring instant load times. Technical foundation: code splitting and lazy loading reduce bundle sizes 60-70%, CDN distribution with edge caching delivers global users <100ms first contentful paint, push notifications re-engage users with precision targeting, and IndexedDB/localStorage enable rich offline experiences with automatic sync when connectivity returns. Business impact: 45% average increase in mobile conversion rates, 80% reduction in data consumption vs native apps, eliminated App Store review cycles, cross-platform code reuse reduces development time 40%. We've built PWAs for retail, utilities, logistics, and financial services—platforms with 500K+ daily active users and 90%+ install rates.`,
                        tags: ["Service Workers", "Offline-First", "Web Manifest", "Push Notifications", "Code Splitting", "Global CDN"],
                    },
                    {
                        id: "CMS",
                        target: "CMS",
                        title: "Content Management & Publishing",
                        body: `Headless CMS platforms decoupling content management from presentation, enabling content teams to publish across unlimited channels (web, mobile, social, email, IoT) from a single system. Our CMS architecture includes: flexible content modeling with custom fields, relationships, and workflows; visual content builder with WYSIWYG editors and markdown support; editorial workflows with role-based approval chains and scheduling; multi-language support with automatic translations; asset management with image optimization, format conversion, and CDN delivery; SEO toolkit with metadata management, structured data generation, and sitemap automation; versioning and rollback capability with audit trails; API-first design (GraphQL and REST) for decoupled frontends; webhooks for real-time event triggers and third-party integrations. Performance: 1M+ API requests/day capacity, millisecond query response times, automatic caching layers reduce origin load 85%. Integration with design systems, analytics platforms, and marketing automation tools. Proven with media companies publishing 10K+ articles, retailers managing 1M+ product descriptions, and enterprises coordinating global content operations.`,
                        tags: ["Headless Architecture", "Visual Builder", "Multi-language", "Workflows", "GraphQL API", "SEO Toolkit", "Asset Optimization"],
                    },
                    {
                        id: "ECOM",
                        target: "ECOM",
                        title: "E-commerce & Storefronts",
                        body: `High-performance e-commerce platforms engineered for conversion and operational excellence. Our storefronts feature: responsive product catalog with faceted search, real-time inventory sync, dynamic pricing and promotion engines, sophisticated shopping cart with saved items and wishlists, one-click checkout with guest mode, multiple payment processor integrations (Stripe, Square, PayPal, Klarna), secure PCI DSS Level 1 compliance, abandoned cart recovery with email automation, order management system with fulfillment tracking, customer accounts with order history and saved preferences, and analytics dashboards measuring conversion funnel, AOV, and customer lifetime value. Technical excellence: <2 second page load times globally, sub-100ms product search, real-time stock updates preventing oversells, fraud detection integrations, internationalization for multi-currency/multi-language selling. Business metrics: average 35% increase in conversion rate vs legacy platforms, 60% faster page load = 12% revenue uplift, 40% reduction in cart abandonment with targeted recovery. Deployed for direct-to-consumer brands doing $10M+/year, multi-brand marketplaces, and subscription box services.`,
                        tags: ["Product Catalog", "Smart Search", "Payment Integration", "PCI Compliance", "Conversion Optimization", "Fulfillment", "Analytics"],
                    },
                    {
                        id: "COLLAB",
                        target: "COLLAB",
                        title: "Collaboration & Productivity",
                        body: `Real-time collaboration platforms enabling teams to work seamlessly across continents with instant synchronization. We architect WebSocket-powered infrastructure with operational transformation (OT) or conflict-free replicated data types (CRDTs) for simultaneous editing without conflicts. Features include: real-time document/spreadsheet editing with presence awareness (see who's viewing/editing), infinite undo/redo, comment threads with @mentions and resolution workflows, shared spaces for projects/teams, task management with dependencies and Gantt views, chat with rich formatting and search, voice/video calling (WebRTC), file sharing with version control, and permissions granularity down to cell/row level. Performance: sub-50ms synchronization latency across global regions, handle 1000+ concurrent users per workspace, automatic conflict resolution, offline mode with sync-on-reconnect. Integrations: Slack notifications, calendar sync (Outlook, Google), identity providers, analytics tools. Proven with agencies collaborating across timezones, distributed development teams, consulting firms, and creative teams—serving 2M+ monthly active users.`,
                        tags: ["Real-time Sync", "WebRTC", "Presence Awareness", "CRDT", "Global Infrastructure", "Video Calling", "Permissions"],
                    },
                    {
                        id: "ANALYTICS",
                        target: "ANALYTICS",
                        title: "Analytics & Business Intelligence",
                        body: `Real-time analytics dashboards transforming raw data into actionable intelligence for C-suite and operational teams. We design custom dashboards with interactive visualizations: time-series charts, geographic heatmaps, cohort analysis, funnel visualizations, and custom metric builders. Data layer handles massive scale: ingest 100B+ events daily, query Petabytes of historical data in seconds, drill-down capabilities from aggregate metrics to individual users. Features: automated alerting when KPIs breach thresholds, scheduled reports emailed to stakeholders, predictive analytics with ML-driven forecasting, anomaly detection identifying unusual patterns, segmentation and cohort analysis, attribution modeling connecting touchpoints to outcomes, A/B testing framework with statistical significance. Technical architecture: stream processing for real-time aggregation, columnar databases (ClickHouse, DuckDB) for query performance, dimensional data modeling (star schema), data lineage and governance. Integrations: data warehouses (Snowflake, BigQuery, Redshift), BI tools, marketing automation, product analytics. Deployed for B2B SaaS tracking product adoption, e-commerce platforms monitoring conversion funnels, fintech firms tracking fraud indicators, and enterprises running data-driven decision-making.`,
                        tags: ["Real-time Dashboards", "Stream Processing", "Predictive Analytics", "Anomaly Detection", "ML Forecasting", "Data Governance", "Drill-down Analysis"],
                    },
                ]}
            />
