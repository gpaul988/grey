    ...
    <main className="relative min-h-screen pb-24 pt-28">
      ...
      <div className="relative mx-auto max-w-5xl px-4 sm:px-6">
        ...
        >
          ...
          {error && (
            <div className="mt-4 flex items-center gap-3 rounded-xl border border-rose-400/30 bg-rose-400/8 px-4 py-3">
        ...
        {loading && (
          <div className="mt-14 flex flex-col items-center gap-5">
            <div className="relative h-16 w-16">
              ...
              <div className="absolute inset-2 animate-spin rounded-full border-2 border-transparent border-t-indigo-400" style={{ animationDirection: 'reverse', animationDuration: '0.8s' }} />
              <div className="absolute inset-4 rounded-full bg-cyan-400/10" />
            </div>
            <div className="text-center">
              <p className="text-sm font-semibold text-slate-300">Fetching targets and running live checks…</p>
              <p className="mt-1 text-xs text-slate-500">Security · Performance · SEO · Repo · Licensing</p>
            </div>
        ...
        <div className="mb-6 rounded-xl border border-white/10 bg-white/5 p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <p className="text-2xl font-black text-white">{report.overallScore}<span className="text-sm text-slate-500">/100</span></p>
            </div>
          </div>
      ...
      <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/80 to-slate-950/90 p-6 shadow-2xl sm:p-8">
        <div className="flex flex-col items-center gap-8 sm:flex-row">
          ...
          <div className="flex-1 text-center sm:text-left">
            <p className="mb-1 text-xs font-semibold uppercase tracking-widest text-slate-500">Overall Verdict</p>
      ...
      {report.detailedSummary && (
        <div className="mt-5 rounded-2xl border border-white/10 bg-slate-900/60 p-5 sm:p-7">
          ...
          >
            ...
            <span className="flex h-6 w-6 items-center justify-center rounded-full border border-slate-600 text-xs text-slate-400 transition hover:border-cyan-400 hover:text-cyan-400">
              {showDetails ? '−' : '+'}
            </span>
          </button>
          {showDetails && (
            <div className="mt-5 space-y-2 text-sm leading-relaxed text-slate-300">
              {report.detailedSummary.split('\n').map((line, i) => {
                ...
                if (line.startsWith('**') || line.startsWith('##')) {
                  return <p key={i} className="mt-3 font-bold text-white">{clean.replace(/^#+\s*/, '')}</p>;
                }
                if (line.startsWith('-') || line.startsWith('•')) {
      ...
      <div className="mt-10 flex flex-wrap items-center gap-3">
        ...
        >
          {exportStatus.json === 'loading' ? (
            <span className="h-3.5 w-3.5 animate-spin rounded-full border-2 border-indigo-300/40 border-t-indigo-300" />
      ...
      <div className="mt-12 overflow-hidden rounded-2xl border border-cyan-400/30 bg-gradient-to-br from-cyan-500/8 via-indigo-500/5 to-transparent p-8 text-center">
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-3">
          {[
            ...
            { icon: '✉️', label: 'Email', value: 'hello@greyinfotech.com.ng', href: 'mailto:hello@greyinfotech.com.ng' },
          ].map((c) => (
            <a
              key={c.label}
              href={c.href}
    ...
    <div className="rounded-2xl border border-white/10 bg-slate-900/60 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ color, background: color + '1a' }}>
            {section.score}/100
          </span>
        </div>
        <div className="flex items-center gap-3">
