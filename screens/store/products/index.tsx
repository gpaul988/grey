...
export default function ProductsPage() {
    ...
    const priceBands = [
        ...
        { label: '₦500k - ₦1.5M', value: '500000-1500000' },
        { label: 'Premium', value: '1500000' },
    ];

    const { category, brand, search, featured, priceBand } = router.query as Record<string, string>;
    ...
    }, [category, brand, search, featured, priceBand, sort]);
            ...
            <div className="st-card p-5">
                ...
                <div className="space-y-2 text-sm">
                    {priceBands.map((band) => (
                        <button
                            key={band.value}
                            onClick={() => setFilter('priceBand', band.value === 'all' ? null : band.value)}
                            className={`block w-full text-left rounded-xl px-3 py-2 ${priceBand === band.value || (!priceBand && band.value === 'all') ? 'bg-[var(--st-surface-2)] text-[var(--st-teal)]' : 'text-[var(--st-muted)] hover:bg-[var(--st-surface-2)]'}`}
                        >
                            {band.label}
                        </button>
                    ))}
