...
for (const file of targets) {
  ...
  try {
    ...
    if (!/const\s+isDayTime\s*=/.test(src)) {
      // insert after first useState or at top of component function body
      ...
      if (useStateIdx !== -1) {
        ...
        // after first opening brace of component
    ...
    if (!match) {
      console.log(`WARN: no return block matched for ${file}`);
      continue;
    }
    const inner = match[1];
